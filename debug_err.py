...
# from alignment_test import is_aligned
...
class Attention(nn.Module):
    ...
    def _naive_attn(self, x):
        ...
        if self.qk_normalization:
            if self.qk_normalization_head_merged:
                # B, H, N, D
                B_, H_, N_, D_ = q.shape
                tmp=q.transpose(1, 2).flatten(-2, -1)
                print("q:",tmp.device, tmp.dtype, tmp.shape, tmp.stride(), flush=True)
                q = self.q_norm(tmp)[0].view(B_, N_, H_, D_).transpose(1, 2)
                tmp=None
                tmp=k.transpose(1, 2).flatten(-2, -1)
                print("k:",tmp.device, tmp.dtype, tmp.shape, tmp.stride(), flush=True)
                k = self.k_norm(tmp)[0].view(B_, N_, H_, D_).transpose(1, 2)
    ...
    def _flash_attn(self, x, key_padding_mask=None, need_weights=False):
        if self.use_subln:
            ...
            # if not is_aligned(x.data_ptr(), 16):
...
def vit_patch14_flashattn(pretrained=False, **kwargs):
    model = VisionTransformer(
        patch_size=14, embed_dim=1200, depth=4, num_heads=25, mlp_ratio=4, qkv_bias=False,
        xattn=True,
        naiveswiglu=False,
        subln=False,
        rope=False,
...
if __name__ == "__main__":
    ...
    for block in model.blocks:
        ...
        block.mlp = deepspeed.moe.layer.MoE(hidden_size=feat_in, expert=block.mlp, k=1,
                                            num_experts=ep_size, ep_size=ep_size,
                                            drop_tokens=True,
                                            buggy=True) # buggy=True to reproduce
    mode = model.cuda().bfloat16()
    ...
    for i in range(2):
        ...
        # out.mean().backward       # if not use deepspeed, also works well
